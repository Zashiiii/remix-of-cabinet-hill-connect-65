  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      const { error } = await supabase.from("residents")
        .update({ contact_number: contactNumber, updated_at: new Date().toISOString() })
        .eq("id", profile.id);
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Contact staff to update your name</p>
          </div>
          <div className="space-y-2">
            <Label htmlFor="contactNumber">Contact Number</Label>
            <Input id="contactNumber" value={contactNumber} onChange={(e) => setContactNumber(e.target.value)} placeholder="09123456789" />
          </div>
